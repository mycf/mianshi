...
j```
## put插入
jjj![[HashMap 2024-01-06 14.11.10.excalidraw|100%]]
## 数组扩容
![[HashMap 2024-01-06 15.31.21.excalidraw|100%]]
## resize
### 总结
J2倍;·JDK1.8 同样是先生成新数组，长度是老数组的
遍历老数组桶中的每个元素;
·如果桶节点没有形成链表，计算出新数组的索位置，直接转移到新数组;如果桶节点已经形成链表
将链表重新链接，按照低位区和高位区重亲斤分配到新数组;·如果桶节点已经形成红黑树
调用split方法将红黑树重新切分为低位区和高位区2个链表;
小于6，则会进行取消树化的处理，否则会将新生成的链表重新树化;判断低位区和高位区链表的长度，链表长ap对象的table属性。·所有元素转移完之后，将新数组赋值给HashM
# 1.7
